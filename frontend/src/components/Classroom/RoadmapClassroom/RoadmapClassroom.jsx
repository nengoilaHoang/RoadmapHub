...
export default function RoadmapClassroom(props) {
  ...
  const handleAddRoadmap = async () => {
    ...
    {
      const response = await api.post('/classrooms/addRoadmapIntoClass', { classroomId: classroomId, roadmapId: selectedRoadmap.id }, {
      withCredentials: true
      });
    const getRoadmap = await api.get('/roadmaps/getTopicRoadmapByUserId', { params: { id: selectedRoadmap.id }, withCredentials: true })
    setRoadmaps([getRoadmap]);
    }
      ...
      {step === 1 && (
        <>
          ...
          <div className="input-group mb-3 ">
            ...
            <button type="button" className="btn btn-primary" onClick={handleAddRoadmap}>
              + Add Roadmap
            </button>
          </div>
